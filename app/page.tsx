...
export default function Page() {
  const [form, setForm] = useState({
    // Tiers are auto-derived from each spouse's program year.
    tierA: "1",
    tierB: "1",
  ...
  // Auto-derive each spouse's tier from that spouse's program year.
  useEffect(() => {
    const autoTierA = PROGRAM_YEAR_TO_TIER[form.programYearA] || "1";
    const autoTierB = PROGRAM_YEAR_TO_TIER[form.programYearB] || "1";

    if (form.tierA !== autoTierA || form.tierB !== autoTierB) {
      setForm((prev) => ({
        ...prev,
        tierA: autoTierA,
        tierB: autoTierB,
  ...
  }, [form.programYearA, form.programYearB, form.tierA, form.tierB]);
  ...
  async function run() {
    ...
    try {
      const payload: any = {
        // Keep sending tier values for compatibility/debugging. The sheet derives the real tiers in B9 and B11.
        tier: form.tierA,
        tierA: form.tierA,
        tierB: hideSpouseB ? "" : form.tierB,
        ...
        programYear: form.programYearA,
        programYearA: form.programYearA,
        programYearB: hideSpouseB ? "" : form.programYearB,
        kidsP8: form.kidsP8,
        ...
        <div style={{ marginTop: 16, display: "grid", gap: 16 }}>
          ...
          <div style={{ ...surface, padding: 16 }}>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(12, 1fr)", gap: 12 }}>
              <div style={{ gridColumn: hideSpouseB ? "span 4" : "span 3" }}>
              ...
              <div style={{ gridColumn: hideSpouseB ? "span 4" : "span 3" }}>
                <Field label="Spouse A Program Year">
                  <select
                    style={control}
                    value={form.programYearA}
                    onChange={(e) => setForm({ ...form, programYearA: e.target.value })}
                  >
              ...
              <div style={{ gridColumn: hideSpouseB ? "span 4" : "span 2" }}>
                <Field label="Tier A">
                  <div
                    style={{
                      ...control,
              ...
              {!hideSpouseB && (
                <>
                  <div style={{ gridColumn: "span 3" }}>
                    <Field label="Spouse B Program Year">
                      <select
                        style={control}
                        value={form.programYearB}
                        onChange={(e) => setForm({ ...form, programYearB: e.target.value })}
                      >
                        {OPTIONS.programYears.map((y) => (
                          <option key={y} value={y}>
                            {y}
